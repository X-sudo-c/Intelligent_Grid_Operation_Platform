-- Faster endpoint geometry scan: temp GiST per district + hash anti-join on proposals.

CREATE INDEX IF NOT EXISTS idx_conductor_endpoint_proposals_active_segment
...
BEGIN
  ...
  v_scan_cap := LEAST(GREATEST(p_limit, 1000), 5000);
  ...
  DROP TABLE IF EXISTS _scan_district_assets;
  CREATE TEMP TABLE _scan_district_assets ON COMMIT DROP AS
  SELECT
    l.unique_id,
    l.geom,
  ...
  CREATE INDEX _scan_district_assets_geom_idx ON _scan_district_assets USING gist (geom);
  ANALYZE _scan_district_assets;
  ...
  WITH unpromoted AS (
    SELECT
      ...
      ST_StartPoint(cs.geom) AS start_pt,
      ST_EndPoint(cs.geom) AS end_pt,
    ...
    LEFT JOIN gis.conductor_endpoint_proposals p
      ON p.segment_id = cs.id
     AND p.status IN ('pending', 'approved')
    WHERE s.reason <> 'already_promoted'
      AND cs.district = v_district
      AND cs.geom IS NOT NULL
      AND p.id IS NULL
      AND (src_l.unique_id IS NULL OR tgt_l.unique_id IS NULL)
    ORDER BY cs.id
    LIMIT v_scan_cap
  ),
  nearest AS (
    ...
    LEFT JOIN LATERAL (
      SELECT
        da.unique_id,
        ST_Distance(u.start_pt::geography, da.geom::geography) AS dist_m,
        da.asset_kind
      FROM _scan_district_assets da
      ORDER BY da.geom <-> u.start_pt
      LIMIT 1
    ) ns ON NOT u.start_ok
    LEFT JOIN LATERAL (
      SELECT
        da.unique_id,
        ST_Distance(u.end_pt::geography, da.geom::geography) AS dist_m,
        da.asset_kind
      FROM _scan_district_assets da
      ORDER BY da.geom <-> u.end_pt
      LIMIT 1
    ) ne ON NOT u.end_ok
  ),
  resolved AS (
