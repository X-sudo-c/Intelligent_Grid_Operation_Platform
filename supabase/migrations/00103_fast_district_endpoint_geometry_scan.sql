-- Speed up endpoint fix geometry scan: district-scoped KNN + skip already-queued segments + batch cap.
-- Previously scanned ALL unpromoted rows nationally (28k+ KNN × 2 per district → hours).

CREATE INDEX IF NOT EXISTS idx_conductor_endpoint_proposals_segment_status
  ON gis.conductor_endpoint_proposals (segment_id, status);

CREATE INDEX IF NOT EXISTS idx_conductor_segments_district_id
  ON gis.conductor_segments (district, id)
  WHERE district IS NOT NULL;

CREATE OR REPLACE FUNCTION gis.nearest_district_endpoint_asset(
  p_district TEXT,
  p_point geometry
)
RETURNS TABLE(
...
AS $$
  SELECT
    l.unique_id,
    ST_Distance(p_point::geography, l.geom::geography),
    am.source_layer,
    gis.endpoint_asset_kind(am.source_layer)
  FROM gis.district_endpoint_lookup l
  LEFT JOIN LATERAL (
    SELECT a.source_layer
    FROM gis.asset_id_map a
    WHERE btrim(a.source_unique_id) = l.unique_id
      AND a.geom IS NOT NULL
    LIMIT 1
  ) am ON TRUE
  WHERE l.district = NULLIF(btrim(p_district), '')
    AND l.geom IS NOT NULL
    AND l.unique_id IS NOT NULL
    AND btrim(l.unique_id) <> ''
  ORDER BY l.geom <-> p_point
  LIMIT 1;
$$;

COMMENT ON FUNCTION gis.nearest_district_endpoint_asset IS
  'KNN endpoint asset within one ECG district (uses district_endpoint_lookup, not national asset_id_map).';
...
BEGIN
  ...
  v_scan_cap := LEAST(GREATEST(p_limit / 2, 500), 2500);
  ...
  WITH district_assets AS MATERIALIZED (
    SELECT
      l.unique_id,
      l.geom,
      'connectivity_asset'::TEXT AS asset_kind
    FROM gis.district_endpoint_lookup l
    WHERE l.district = v_district
      AND l.geom IS NOT NULL
      AND l.unique_id IS NOT NULL
      AND btrim(l.unique_id) <> ''
  ),
  unpromoted AS (
    ...
    WHERE s.reason <> 'already_promoted'
      AND cs.district = v_district
      AND cs.geom IS NOT NULL
      AND NOT EXISTS (
        SELECT 1
        FROM gis.conductor_endpoint_proposals p
        WHERE p.segment_id = cs.id
          AND p.status IN ('pending', 'approved')
      )
    ORDER BY
      CASE WHEN src_l.unique_id IS NULL OR tgt_l.unique_id IS NULL THEN 0 ELSE 1 END,
      cs.id
    LIMIT v_scan_cap
  ),
  nearest AS (
    ...
    LEFT JOIN LATERAL (
      SELECT
        da.unique_id,
        ST_Distance(ST_StartPoint(u.line_geom)::geography, da.geom::geography) AS dist_m,
        da.asset_kind
      FROM district_assets da
      ORDER BY da.geom <-> ST_StartPoint(u.line_geom)
      LIMIT 1
    ) ns ON NOT u.start_ok
    LEFT JOIN LATERAL (
      SELECT
        da.unique_id,
        ST_Distance(ST_EndPoint(u.line_geom)::geography, da.geom::geography) AS dist_m,
        da.asset_kind
      FROM district_assets da
      ORDER BY da.geom <-> ST_EndPoint(u.line_geom)
      LIMIT 1
    ) ne ON NOT u.end_ok
  ),
  resolved AS (
...
GRANT EXECUTE ON FUNCTION gis.nearest_district_endpoint_asset(TEXT, geometry) TO service_role;
